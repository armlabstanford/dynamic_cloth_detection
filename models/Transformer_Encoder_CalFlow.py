...
class TransformerModel(nn.Module):
    ...
    def forward(self, cal_image_data):
        ...
        combined_features = cal_image_features
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = TransformerModel(d_model=60, nhead=6, num_encoder_layers=3, dim_feedforward=2048, num_classes=4).to(device) # use this if NOT using uncalibrated optical flow
