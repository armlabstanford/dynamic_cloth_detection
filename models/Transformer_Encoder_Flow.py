...
train_dataset = TensorDataset(train_data_tensor, uncalibrated_train_data_tensor, labels_train)
val_dataset = TensorDataset(val_data_tensor, uncalibrated_val_data_tensor, labels_val)
test_dataset = TensorDataset(test_data_tensor, uncalibrated_test_data_tensor, labels_test)
...
class TransformerModel(nn.Module):
    ...
    def forward(self, cal_image_data, uncal_image_data): 
        ...
        combined_features = torch.cat((cal_image_features, uncal_image_features), dim=2)
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = TransformerModel(d_model=60, nhead=8, num_encoder_layers=3, dim_feedforward=2048, num_classes=4).to(device) # use this if using uncalibrated optical flow
...
for epoch in range(max_epochs):
    ...
    for cal_image_inputs, uncal_image_inputs, labels in train_loader:
        optimizer.zero_grad()
        outputs, _ = model(cal_image_inputs.to(device), uncal_image_inputs.to(device))
    ...
    with torch.no_grad():
        for cal_image_inputs, uncal_image_inputs, labels in val_loader:
            outputs, _ = model(cal_image_inputs.to(device), uncal_image_inputs.to(device))
            loss = criterion(outputs, labels.to(device))
...
def evaluate(model, test_loader, criterion, device):
    ...
    with torch.no_grad():  # Disable gradient computation
        for cal_image_inputs, uncal_image_inputs, labels in test_loader:
            cal_image_inputs, uncal_image_inputs, labels = cal_image_inputs.to(device), uncal_image_inputs.to(device), labels.to(device)
            outputs, _ = model(cal_image_inputs, uncal_image_inputs)
...
def extract_latent_features(model, data_loader, device):
    ...
    with torch.no_grad():
        for cal_image_inputs, uncal_image_inputs, labels in data_loader:
            cal_image_inputs, uncal_image_inputs, labels = cal_image_inputs.to(device), uncal_image_inputs.to(device), labels.to(device)
            _, latent = model(cal_image_inputs, uncal_image_inputs)        
