...
class TransformerModel(nn.Module):
    ...
    def forward(self, cal_image_data, uncal_image_data, force_data): # for calibrated optical flow, uncalibrated optical flow, wrench
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = TransformerModel(d_model=60, nhead=8, num_encoder_layers=3, dim_feedforward=2048, num_classes=4, force_input_size=6, force_output_size=24).to(device) # use this if using uncalibrated optical flow
