...
class TransformerModel(nn.Module):
    ...
    def forward(self, cal_image_data, uncal_image_data, joint_data): # for calibrated optical flow, uncalibrated optical flow, wrench, and joint states
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = TransformerModel(d_model=60, nhead=8, num_encoder_layers=3, dim_feedforward=2048, num_classes=4, joint_input_size=4, joint_output_size=16).to(device) 
