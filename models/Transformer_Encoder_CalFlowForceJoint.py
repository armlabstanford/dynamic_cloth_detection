...
train_dataset = TensorDataset(train_data_tensor,  forces_train_data_tensor, joint_state_train_data_tensor, labels_train)
val_dataset = TensorDataset(val_data_tensor, forces_val_data_tensor, joint_state_val_data_tensor, labels_val)
test_dataset = TensorDataset(test_data_tensor, forces_test_data_tensor, joint_state_test_data_tensor, labels_test)
...
class TransformerModel(nn.Module):
    def __init__(self, d_model, nhead, num_encoder_layers, dim_feedforward, num_classes, force_input_size, force_output_size, joint_input_size, joint_output_size):
        ...
        self.pos_encoder = nn.Parameter(torch.zeros(1, 200, d_model + force_output_size + joint_output_size)) 
        encoder_layers = nn.TransformerEncoderLayer(d_model + force_output_size + joint_output_size, nhead, dim_feedforward) 
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_encoder_layers)
        self.fc1 = nn.Linear(d_model + force_output_size + joint_output_size, 40) 
        self.fc2 = nn.Linear(40, num_classes)
        self.d_model = d_model

    def forward(self, cal_image_data, force_data, joint_data): 
        ...
        combined_features = torch.cat((cal_image_features, force_features, joint_features), dim=2)
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = TransformerModel(d_model=60, nhead=7, num_encoder_layers=3, dim_feedforward=2048, num_classes=4, force_input_size=6, force_output_size=6, joint_input_size=4, joint_output_size=4).to(device) 
...
for epoch in range(max_epochs):
    ...
    for cal_image_inputs, force_inputs, joint_inputs, labels in train_loader:
        optimizer.zero_grad()
        outputs, _ = model(cal_image_inputs.to(device), force_inputs.to(device), joint_inputs.to(device))
    ...
    with torch.no_grad():
        for cal_image_inputs, force_inputs, joint_inputs, labels in val_loader:
            outputs, _ = model(cal_image_inputs.to(device), force_inputs.to(device), joint_inputs.to(device))
            loss = criterion(outputs, labels.to(device))
...
def evaluate(model, test_loader, criterion, device):
    ...
    with torch.no_grad():  # Disable gradient computation
        for cal_image_inputs, force_inputs, joint_inputs, labels in test_loader:
            cal_image_inputs, force_inputs, joint_inputs, labels = cal_image_inputs.to(device), force_inputs.to(device), joint_inputs.to(device), labels.to(device)
            outputs, _ = model(cal_image_inputs, force_inputs, joint_inputs)
...
def extract_latent_features(model, data_loader, device):
    ...
    with torch.no_grad():
        for cal_image_inputs, force_inputs, joint_inputs, labels in data_loader:
            cal_image_inputs, force_inputs, joint_inputs, labels = cal_image_inputs.to(device), force_inputs.to(device), joint_inputs.to(device), labels.to(device)
            _, latent = model(cal_image_inputs, force_inputs, joint_inputs)        
