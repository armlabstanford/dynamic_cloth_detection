...
# repeat for flow angles
calibrated_flow_ang_0 = np.load(f'{data_path}/calibrated_flow_data_0_layers.npz')['flow_data'][:,1,:,:,:]
print(f"Flow angle data for 0 layers has shape {calibrated_flow_ang_0.shape}")

calibrated_flow_ang_1 = np.load(f'{data_path}/calibrated_flow_data_1_layers.npz')['flow_data'][:,1,:,:,:]
print(f"Flow angle data for 1 layers has shape {calibrated_flow_ang_1.shape}")

calibrated_flow_ang_2 = np.load(f'{data_path}/calibrated_flow_data_2_layers.npz')['flow_data'][:,1,:,:,:]
print(f"Flow angle data for 2 layers has shape {calibrated_flow_ang_2.shape}")
...
# repeat for flow angles
train_ang_0 = calibrated_flow_ang_0[:int(0.70*num_trials_0)]
val_ang_0 = calibrated_flow_ang_0[int(0.70*num_trials_0):int(0.85*num_trials_0)]
test_ang_0 = calibrated_flow_ang_0[int(0.85*num_trials_0):]

train_ang_1 = calibrated_flow_ang_1[:int(0.70*num_trials_1)]
val_ang_1 = calibrated_flow_ang_1[int(0.70*num_trials_1):int(0.85*num_trials_1)]
test_ang_1 = calibrated_flow_ang_1[int(0.85*num_trials_1):]

train_ang_2 = calibrated_flow_ang_2[:int(0.70*num_trials_2)]
val_ang_2 = calibrated_flow_ang_2[int(0.70*num_trials_2):int(0.85*num_trials_2)]
test_ang_2 = calibrated_flow_ang_2[int(0.85*num_trials_2):]

train_ang_3 = calibrated_flow_ang_3[:int(0.70*num_trials_3)]
val_ang_3 = calibrated_flow_ang_3[int(0.70*num_trials_3):int(0.85*num_trials_3)]
test_ang_3 = calibrated_flow_ang_3[int(0.85*num_trials_3):]

print("Calibrated Camera Angles:")
print(f"Training angle data for 0 layers has shape {train_ang_0.shape}")
print(f"Validation angle data for 0 layers has shape {val_ang_0.shape}")
print(f"Testing angle data for 0 layers has shape {test_ang_0.shape}")

print(f"Training angle data for 1 layers has shape {train_ang_1.shape}")
print(f"Validation angle data for 1 layers has shape {val_ang_1.shape}")
print(f"Testing angle data for 1 layers has shape {test_ang_1.shape}")

print(f"Training angle data for 2 layers has shape {train_ang_2.shape}")
print(f"Validation angle data for 2 layers has shape {val_ang_2.shape}")
print(f"Testing angle data for 2 layers has shape {test_ang_2.shape}")

print(f"Training angle data for 3 layers has shape {train_ang_3.shape}")
print(f"Validation angle data for 3 layers has shape {val_ang_3.shape}")
print(f"Testing angle data for 3 layers has shape {test_ang_3.shape}")
...
# Concatenate and label train angle data
train_angle_data = np.concatenate((train_ang_0, train_ang_1, train_ang_2, train_ang_3), axis=0)
train_angle_data_tensor = torch.tensor(train_angle_data).float().to(device)

# Concatenate and label validation angle data
val_angle_data = np.concatenate((val_ang_0, val_ang_1, val_ang_2, val_ang_3), axis=0)
val_angle_data_tensor = torch.tensor(val_angle_data).float().to(device)

# Concatenate and label test angle data
test_angle_data = np.concatenate((test_ang_0, test_ang_1, test_ang_2, test_ang_3), axis=0)
test_angle_data_tensor = torch.tensor(test_angle_data).float().to(device)
...
train_dataset = TensorDataset(train_data_tensor, train_angle_data_tensor, labels_train)
val_dataset = TensorDataset(val_data_tensor, val_angle_data_tensor, labels_val)
test_dataset = TensorDataset(test_data_tensor, test_angle_data_tensor, labels_test)
...
class TransformerModel(nn.Module):
    def __init__(self, d_model, nhead, num_encoder_layers, dim_feedforward, num_classes):
        ...
        self.pos_encoder = nn.Parameter(torch.zeros(1, 200, 2*d_model)) 
        encoder_layers = nn.TransformerEncoderLayer(2*d_model, nhead, dim_feedforward) 
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_encoder_layers)
        self.fc1 = nn.Linear(2*d_model, 40) 
        self.fc2 = nn.Linear(40, num_classes)
        self.d_model = d_model

    def forward(self, cal_image_data, cal_image_angle_data):
        batch_size, seq_len, _, _, = cal_image_data.shape

        # calibrated optical flow mag
        ...
        # calibrated optical flow angle
        cal_image_angle_data = cal_image_angle_data.view(batch_size * seq_len, 1, 96, 128)
        cal_image_angle_features, _ = self.feature_extractor(cal_image_angle_data)
        cal_image_angle_features = cal_image_angle_features.view(batch_size, seq_len, -1)

        combined_features = torch.cat((cal_image_features, cal_image_angle_features), dim=2)
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = TransformerModel(d_model=60, nhead=8, num_encoder_layers=3, dim_feedforward=2048, num_classes=4).to(device) 
...
for epoch in range(max_epochs):
    ...
    for cal_image_inputs, cal_image_angle_inputs, labels in train_loader:
        optimizer.zero_grad()
        outputs, _ = model(cal_image_inputs.to(device), cal_image_angle_inputs.to(device))
    ...
    with torch.no_grad():
        for cal_image_inputs, cal_image_angle_inputs, labels in val_loader:
            outputs, _ = model(cal_image_inputs.to(device), cal_image_angle_inputs.to(device))
            loss = criterion(outputs, labels.to(device))
...
def evaluate(model, test_loader, criterion, device):
    ...
    with torch.no_grad():  # Disable gradient computation
        for cal_image_inputs, cal_image_angle_inputs, labels in test_loader:
            cal_image_inputs, cal_image_angle_inputs, labels = cal_image_inputs.to(device), cal_image_angle_inputs.to(device), labels.to(device)
            outputs, _ = model(cal_image_inputs, cal_image_angle_inputs)
...
def extract_latent_features(model, data_loader, device):
    ...
    with torch.no_grad():
        for cal_image_inputs, cal_image_angle_inputs, labels in data_loader:
            cal_image_inputs, cal_image_angle_inputs, labels = cal_image_inputs.to(device), cal_image_angle_inputs.to(device), labels.to(device)
            _, latent = model(cal_image_inputs, cal_image_angle_inputs)        
