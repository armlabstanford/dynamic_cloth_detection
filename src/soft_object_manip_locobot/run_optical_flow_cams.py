...
def run_optical_flow_instance(camera_index, filename):
    args = ['python', 'optical_flow_without_ros.py', camera_index, filename]
...
def main():
    ...
    arguments = [
        ("0", "tests_3/three_layer"),
        ("2", "tests_3/three_layer")
    ]
    ...
    time.sleep(2.0)
    thread = subprocess.run(['python', 'dynamixel_finger/rubbing_motion.py'])
    threads.append(thread)
